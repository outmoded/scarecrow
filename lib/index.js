'use strict';

const Hoek = require('hoek');
const Oz = require('oz');


const internals = {};


internals.defaults = {

    // Oz options

    oz: {
        encryptionPassword: null,
        loadAppFunc: null,
        loadGrantFunc: null,
        hawk: null,
        ticket: null
    },

    // Scarecrow options

    urls: {
        app: '/oz/app',
        reissue: '/oz/reissue',
        rsvp: '/oz/rsvp'
    }
};


exports.plugin = {
    pkg: require('../package.json'),
    requirements: {
        hapi: '>=17.7.0'
    },
    register: function (server, options) {

        server.auth.scheme('oz', internals.oz);
    }
};


internals.oz = function (server, options) {

    Hoek.assert(options, 'Invalid hawk scheme options');
    Hoek.assert(options.oz, 'Missing required oz configuration');

    const settings = Hoek.applyToDefaults(internals.defaults, options);

    // Add protocol endpoints

    const endpoint = (name) => {

        const route = {
            auth: false,                            // Override any defaults
            handler: function (request) {

                return Oz.endpoints[name](request.raw.req, request.payload, settings.oz);
            }
        };

        return route;
    };

    server.route([
        { method: 'POST', path: settings.urls.app, config: endpoint('app') },
        { method: 'POST', path: settings.urls.reissue, config: endpoint('reissue') },
        { method: 'POST', path: settings.urls.rsvp, config: endpoint('rsvp') }
    ]);

    const scheme = {
        api: { settings },
        authenticate: async function (request, h) {

            const { ticket, artifacts } = await Oz.server.authenticate(request.raw.req, settings.oz.encryptionPassword, { hawk: settings.oz.hawk });
            return h.authenticated({ credentials: ticket, artifacts });
        }
    };

    return scheme;
};
